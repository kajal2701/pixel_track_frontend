...
import { Phone, Email, LocationOn, ArrowBack } from '@mui/icons-material';
import toast from 'react-hot-toast';
import invoiceService from 'src/services/invoiceService';
import Logo from 'src/assets/images/logos/PiXEL-Tracks-Lights_Logo-White.webp';
import PaymentDialog from 'src/views/admin/invoices/invoicelist/invoicelisting/PaymentDialog';
import PageContainer from 'src/components/container/PageContainer';
import { formatDate, PIXEL_TRACK, decodeInvoiceToken, getInvoiceStatusDisplay } from 'src/utils/helpers';
...
const CustomerInvoiceView = () => {
  ...
  const fetchInvoiceDetails = async (actualId) => {
    ...
    setError('');
    try {
      const res = await invoiceService.getCustomerInvoiceById(actualId, customer.id);
      setInvoice(res.data);
    } catch (err) {
  ...
  if (loading) {
    return (
      <PageContainer title="Loading Invoice" description="Loading invoice details">
  ...
  if (error || !invoice) {
    return (
      <PageContainer title="Invoice" description="Invoice details">
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '60vh', p: 3 }}>
          <Typography variant="h5" color="error" gutterBottom>{error || 'Invoice Not Found'}</Typography>
          <Button
            variant="contained"
            color="primary"
            startIcon={<ArrowBack />}
  ...
  return (
    <PageContainer title={`Invoice ${invoice.invoice_number}`} description="View invoice details">
      <style>{`
        @media print {
          * {
        ...
        <Box sx={{ p: { xs: 3, md: 5 } }}>
          ...
          <Box className="no-print" sx={{ mt: 4, pt: 3, borderTop: '1px solid', borderColor: 'divider' }}>
            <Stack direction="row" spacing={2} justifyContent="center">
              ...
              <Button
                id="btn-customer-download-invoice"
                variant="outlined"
                color="primary"
